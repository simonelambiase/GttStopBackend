...
import it.simonelambiase.StopGttService.mato.model.response.stop.StopRoutes;
...
public class MatoService {
    ...
    private final String codedIdQuery = "{\"id\":\"q01\",\"query\":\"query StopRoutes($id_0:String!,$startTime_1:Long!,$timeRange_2:Int!,$numberOfDepartures_3:Int!) {stop(id:$id_0) {id,...F2}} fragment F0 on Alert {id,alertDescriptionText,alertHash,alertHeaderText,alertSeverityLevel,alertUrl,effectiveEndDate,effectiveStartDate,alertDescriptionTextTranslations {language,text},alertHeaderTextTranslations {language,text},alertUrlTranslations {language,text}} fragment F1 on Route {alerts {trip {pattern {code,id},id},id,...F0},id} fragment F2 on Stop {_stoptimesWithoutPatterns2LPFJd:stoptimesWithoutPatterns(startTime:$startTime_1,timeRange:$timeRange_2,numberOfDepartures:$numberOfDepartures_3,omitCanceled:false) {realtimeState,trip {pattern {code,id},route {gtfsId,shortName,longName,mode,color,id,...F1},id}},id}\",\"variables\":{\"id_0\":\"gtt:$stopToReplace\",\"startTime_1\":$startTimeToReplace,\"timeRange_2\":900,\"numberOfDepartures_3\":100}}";
    private final String nextPassagesQuery = "{\"id\":\"q02\",\"query\":\"query StopPageContentContainer_StopRelayQL($id_0:ID!,$startTime_1:Long!,$timeRange_2:Int!,$numberOfDepartures_3:Int!) {node(id:$id_0) {...F2}} fragment F0 on Route {alerts {alertSeverityLevel,effectiveEndDate,effectiveStartDate,trip {pattern {code,id},id},id},id} fragment F1 on Stoptime {realtimeState,realtimeDeparture,scheduledDeparture,realtimeArrival,scheduledArrival,realtime,serviceDay,pickupType,stopHeadsign,stop {id,code,platformCode},trip {gtfsId,directionId,tripHeadsign,wheelchairAccessible,stops {id},pattern {route {gtfsId,shortName,longName,mode,color,agency {name,id},id,...F0},code,stops {gtfsId,code,id},id},id}} fragment F2 on Stop {_stoptimesWithoutPatterns1svkTi:stoptimesWithoutPatterns(startTime:$startTime_1,timeRange:$timeRange_2,numberOfDepartures:$numberOfDepartures_3,omitCanceled:false) {...F1},id}\",\"variables\":{\"id_0\":\"$idToReplace\",\"startTime_1\":\"$startTimeToReplace\",\"timeRange_2\":$timeRangeToReplace,\"numberOfDepartures_3\":100}}\n";
    public List<BusPassage> retrieveBusPassage ( String stopNumber, int nextHours ) {
    ...
    public List<BusPassage> retrieveBusPassage ( String stopNumber ) {
       return retrieveBusPassage(stopNumber,12);
    }
    protected String retrieveCodedStopId ( String stopNumber ) {
        return retrieveCodedStopId(stopNumber,12);
    }
    protected String retrieveCodedStopId ( String stopNumber, int nextHours ) {
